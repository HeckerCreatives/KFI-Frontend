...
const FormTableDoc = ({ index, entry, remove, form, loading = false }: FSFormTableDocProps) => {
    ...
    <TableRow className="[&>td]:border-2 [&>td]:!py-0 [&>td]:!px-2 [&>td]:!bg-white">
      ...
      <TableCell className="sticky left-0 min-w-[20rem] max-w-[20rem] z-10">
        <div className="flex items-start gap-1">
          <FormIonItem>
            <InputText readOnly control={form.control} name={`entries.${index}.acctCodeDescription`} clearErrors={form.clearErrors} className="!px-2 !min-w-64 rounded-sm" />
          </FormIonItem>
          <div className={classNames('mt-2.5', form?.formState.errors?.entries && form.formState.errors.entries[index]?.acctCodeId && '!mt-0.5')}>
             <ChartOfAccountSelection
                chartOfAccountLabel={`entries.${index}.acctCodeDescription`}
      ...
      <TableCell className="sticky left-[20rem] min-w-[20rem] max-w-[20rem] z-10">
        <FormIonItem>
          <InputText disabled={loading} control={form.control} name={`entries.${index}.debit`} clearErrors={form.clearErrors} className="!px-2 !min-w-64 rounded-sm" isAmount/>
        </FormIonItem>
      </TableCell>
      <TableCell className="sticky left-[20rem] min-w-[20rem] max-w-[20rem] z-10">
        <FormIonItem>
          <InputText disabled={loading} control={form.control} name={`entries.${index}.credit`} clearErrors={form.clearErrors} className="!px-2 !min-w-64 rounded-sm" isAmount />
        </FormIonItem>
      </TableCell>
