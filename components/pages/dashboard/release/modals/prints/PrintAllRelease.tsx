...
const PrintAllRelease = () => {
      ...
      if(contentType.includes('pdf')){
         const fileURL = URL.createObjectURL(blob);
          addJob({
            ...
            fileType: 'pdf',
            file: 'zip',
            filename: `acknowledgement-receipt-${tabActive}.pdf`,
            fileUrl: fileURL
          })
       } else {
        const fileURL = URL.createObjectURL(blob);
  ...
  return (
    <>
      <IonButton fill="clear" id="print_all_release" className="h-10 bg-orange-50 text-orange-500 border border-orange-200 capitalize font-semibold rounded-xl" strong>
      ...
      >
        {/* <IonHeader>
          <IonToolbar className=" text-white [--min-height:1rem] h-12">
            <ModalHeader disabled={loading} title="Acknowledgement - Print All" sub="Transaction" dismiss={dismiss} />
          </IonToolbar>
        </IonHeader> */}
