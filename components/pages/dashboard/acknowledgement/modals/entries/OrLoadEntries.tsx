...
const ORLoadEntries = ({ center, form }: Props) => {
  ...
  const handleSearch = async (page: number) => {
    ...
    try {
      const filter: any = { dueDateId, type };
  ...
  const mapEntryForForm = (entry: Option) => {
    const particular =
      entry.particular ??
      `${entry.centerNo ? `${entry.centerNo} - ` : ''}${entry.name ?? ''}`.trim();
    return {
  ...
  const allSelected =
    data.acknowledgements.length > 0 && selectedIds.length === data.acknowledgements.length;
          ...
          <div className="">
            <div className="flex items-center flex-wrap justify-start gap-2">
              <div className="flex items-center min-w-20 gap-2">
                ...
                {/* Due Date — IonPopover with IonSearchbar + pagination */}
                <DueDateDropdown
                  duedates={duedates}
                  value={dueDateId}
          ...
          <div className="relative overflow-auto">
            <Table>
              ...
              <TableBody>
                ...
                {!data.loading && data.acknowledgements.length < 1 && (
                  <TableNoRows colspan={4} label="No official receipt found" />
                )}
                {!data.loading &&
                  data.acknowledgements.map((entry: Option) => (
                    <TableRow key={entry._id} className="border-b-0 [&>td]:!py-1">
                      ...
                      <TableCell>{entry.cvNo ?? entry.code}</TableCell>
                      <TableCell>{entry.acctCode ?? '-'}</TableCell>
                      <TableCell>{entry.dueDate ? formatDateTable(entry.dueDate) : '-'}</TableCell>
                    </TableRow>
                  ))}
              </TableBody>
            </Table>
