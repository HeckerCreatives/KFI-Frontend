...
const PrintAllAcknowledgement = () => {
 ...
 async function handleDownload(data: PrintExportFilterFormData) {
      ...
      switch (tabActive) {
        case "by-document":
          result = await kfiAxios.get(
            `/official-receipt/print/by-document/${data.option}`,
            { ...config, params }
          );
          break;
        default:
  ...
  return (
    <>
      <IonButton fill="clear" id="print_all_acknowledgement" className="h-10 bg-orange-50 text-orange-500 border border-orange-200 capitalize font-semibold rounded-xl" strong>
