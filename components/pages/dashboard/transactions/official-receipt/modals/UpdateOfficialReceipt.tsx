...
import LoanReleaseForm from '../components/OfficialReceiptForm';
...
const UpdateOfficialReceipt = ({ index }: { index: number }) => {
  ...
  return (
    <>
      <div className="text-end">
        <div
          id={`update-officialReceipt-modal-${index}`}
      ...
      <IonModal
        ref={modal}
        trigger={`update-officialReceipt-modal-${index}`}
        backdropDismiss={false}
        className="auto-height md:[--max-width:90%] md:[--width:100%] lg:[--max-width:70%] lg:[--width:70%]"
      >
        <IonHeader>
          <IonToolbar className=" text-white [--min-height:1rem] h-20">
            <ModalHeader title="Transaction - Official Receipt - Edit Record" sub="All Actions" dismiss={dismiss} />
          </IonToolbar>
        </IonHeader>
...
export default UpdateOfficialReceipt;
