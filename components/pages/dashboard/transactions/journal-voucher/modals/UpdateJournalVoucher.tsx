...
import LoanReleaseForm from '../components/JournalVoucherForm';
import { createSharp } from 'ionicons/icons';
import { JournalVoucherFormData, journalVoucherSchema } from '../../../../../../validations/journal-voucher.schema';

const UpdateJournalVoucher = ({ index }: { index: number }) => {
  ...
  return (
    <>
      <div className="text-end">
        <div
          id={`update-journalVoucher-modal-${index}`}
      ...
      <IonModal
        ref={modal}
        trigger={`update-journalVoucher-modal-${index}`}
        backdropDismiss={false}
        className="auto-height md:[--max-width:90%] md:[--width:100%] lg:[--max-width:70%] lg:[--width:70%]"
      >
        <IonHeader>
          <IonToolbar className=" text-white [--min-height:1rem] h-20">
            <ModalHeader title="Transaction - Journal Voucher - Edit Record" sub="All Actions" dismiss={dismiss} />
          </IonToolbar>
        </IonHeader>
