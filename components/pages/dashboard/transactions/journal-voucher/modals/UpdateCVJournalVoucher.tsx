...
const UpdateCVJournalVoucher = ({ index }: { index: number }) => {
  ...
  return (
    <>
      <div className="text-end">
        <div
          id={`update-cvNoJournalVoucher-modal-${index}`}
      ...
      <IonModal
        ref={modal}
        trigger={`update-cvNoJournalVoucher-modal-${index}`}
        backdropDismiss={false}
        className="auto-height md:[--max-width:90%] md:[--width:100%] lg:[--max-width:70%] lg:[--width:70%]"
      >
        <IonHeader>
          <IonToolbar className=" text-white [--min-height:1rem] h-20">
            <ModalHeader title="Transaction - Journal Voucher - Update CV#" sub="All Actions" dismiss={dismiss} />
          </IonToolbar>
        </IonHeader>
...
export default UpdateCVJournalVoucher;
