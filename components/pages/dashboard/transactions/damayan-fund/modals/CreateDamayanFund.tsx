...
const CreateDamayanFund = () => {
  ...
  function onSubmit(data: DamayanFundFormData) {
    console.log(data);
  }
  ...
  return (
    <>
      <div className="text-end lg:mt-1">
        <IonButton fill="clear" id="create-damayanFund-modal" className="max-h-10 min-h-6 bg-[#FA6C2F] text-white capitalize font-semibold rounded-md" strong>
          + Add Record
        </IonButton>
      </div>
      <IonModal
        ref={modal}
        trigger="create-damayanFund-modal"
        backdropDismiss={false}
        className="auto-height md:[--max-width:90%] md:[--width:100%] lg:[--max-width:70%] lg:[--width:70%]"
      >
        <IonHeader>
          <IonToolbar className=" text-white [--min-height:1rem] h-20">
            <ModalHeader title="Transaction - Damayan Fund - Add Record" sub="All Actions" dismiss={dismiss} />
          </IonToolbar>
        </IonHeader>
