...
type DeleteChartOfAccountProps = {
...
const DeleteChartOfAccount = ({ chartAccount, getChartOfAccounts, searchkey, sortKey, currentPage, rowLength }: DeleteChartOfAccountProps) => {
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      const result = await kfiAxios.delete(`/chart-of-account/${chartAccount._id}`);
      const { success } = result.data;
      if (success) {
        const page = rowLength - 1 === 0 && currentPage > 1 ? currentPage - 1 : currentPage;
        getChartOfAccounts(page, searchkey, sortKey);
        dismiss();
        return;
      }
    } catch (error: any) {
      present({
        message: 'Failed to delete the chart of account record. Please try again',
        duration: 1000,
      });
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <div className="text-end">
        <div
          id={`delete-coa-modal-${chartAccount._id}`}
      ...
      <IonModal
        ref={modal}
        trigger={`delete-coa-modal-${chartAccount._id}`}
        backdropDismiss={false}
        className="auto-height md:[--max-width:90%] md:[--width:100%] lg:[--max-width:50%] lg:[--width:50%]"
      >
        <IonHeader>
          <IonToolbar className=" text-white [--min-height:1rem] h-20">
            <ModalHeader disabled={loading} title="Chart Of Account - Delete Record" sub="Manage Account" dismiss={dismiss} />
          </IonToolbar>
        </IonHeader>
