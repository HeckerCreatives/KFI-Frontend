...
const PrintAllChartOfAccount = () => {
  ...
  setLoading(true);
  try {
    const result = await kfiAxios.get(`/chart-of-account/print-all`, {
 ...
 useEffect(() => {
  ...
  return (
    <>
      <IonButton fill="clear" id="print_all_coa" className="h-10 bg-orange-50 text-orange-500 border border-orange-200 capitalize font-semibold rounded-xl" strong>
        <PrinterIcon size={15} stroke='.8' className=' mr-1'/>
        Print
      </IonButton>
      <IonModal
        ref={modal}
        trigger={`print_all_coa`}
