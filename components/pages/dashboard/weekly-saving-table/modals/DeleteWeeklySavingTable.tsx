...
const DeleteWeeklySavingTable = ({ saving, getWeeklySavings, searchkey, sortKey, currentPage, rowLength }: DeleteWeeklySavingTableProps) => {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (success) {
        ...
        getWeeklySavings(page, searchkey, sortKey);
        dismiss();
        present({
      ...
      present({
        message: 'Failed to delete the weekly saving record. Please try again',
        duration: 1000,
      });
    } finally {
      setLoading(false);
  ...
  return (
    <>
      {/* <div className="text-end">
        <div
          id={`delete-weeklySaving-modal-${saving._id}`}
      ...
      <IonModal
        ref={modal}
        trigger={`delete-weeklySaving-modal-${saving._id}`}
      ...
      >
        {/* <IonHeader>
          <IonToolbar className=" text-white [--min-height:1rem] h-12">
            <ModalHeader disabled={loading} title="Weekly Saving Table - Delete Record" sub="System" dismiss={dismiss} />
          </IonToolbar>
        </IonHeader> */}
        <div className="inner-content !p-6">
            <ModalHeader disabled={loading} title="Weekly Saving Table - Delete Record" sub="Are you sure you want to delete this record?" dismiss={dismiss} />
...
export default DeleteWeeklySavingTable;
