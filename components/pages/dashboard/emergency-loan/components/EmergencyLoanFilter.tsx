...
type EmergencyLoanFilterProps = {
  getEmergencyLoans: (page: number, keyword?: string, sort?: string, from?: string, to?: string) => void;
};

const EmergencyLoanFilter = ({ getEmergencyLoans }: EmergencyLoanFilterProps) => {
  ...
  const onSubmit = (data: TSearch) => {
    if (data.code !== '' || data.sort !== '' || data.dateFrom !== '' || data.dateTo !== '') {
      getEmergencyLoans(1, data.code, data.sort, data.dateTo, data.dateFrom);
    } else {
      getEmergencyLoans(1);
    }
  };
  ...
  const online = useOnlineStore((state) => state.online);
        ...
        const dateFrom = form.watch('dateFrom');
          ...
          useEffect(() => {
            const fetchData = () => {
              if (online) {
                getEmergencyLoans(1, code, sort, dateTo, dateFrom);
              } else {
                getEmergencyLoans(1, code, sort, dateTo, dateFrom);
              }
            };
          ...
          const [data, setData] = useState<TData>({
                      ...
                      useEffect(() => {
                        ...
                        try {
                          const result = await kfiAxios.get('/emergency-loan', { params: { limit: 5, search: code, page: 1 } });
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="w-fit">
       ...
       <FormIonItem className="min-w-32 ![--min-height:1rem] pb-1">
        ...
        <FormIonItem className="min-w-36 ![--min-height:1rem] pb-1">
          ...
          <div className=' flex flex-col gap-1'>
            <label htmlFor="search" className=' text-xs'>Search</label>
              <SearchInput
                  ...
                  suggestions={data.emergencyLoans.map((item) => item.code || '')}
                />
          </div>
...
export default EmergencyLoanFilter;
