...
const PrintAllEmergencyLoan = () => {
  ...
  async function handlePrint(data: EmergencyLoanFilterFormData) {
    try {
      ...
      setLoading(true);
      const result = await kfiAxios.get(`/emergency-loan/print-all/${data.option}`, { params: params, responseType: 'blob' });
  ...
  return (
    <>
      <IonButton fill="clear" id="print_all_emergency_loan" className="max-h-10 w-32 min-w-32 max-w-32 min-h-6 bg-[#FA6C2F] text-white capitalize font-semibold rounded-md" strong>
        <PrinterIcon size={15} stroke='.8' className=' mr-1'/>
        Print All
      </IonButton>
      <IonModal
        ref={modal}
        trigger={`print_all_emergency_loan`}
      ...
      >
        {/* <IonHeader>
          <IonToolbar className=" text-white [--min-height:1rem] h-12">
            <ModalHeader disabled={loading} title="Emergency Loan - Print All" sub="Transaction" dismiss={dismiss} />
          </IonToolbar>
        </IonHeader> */}
        <div className="inner-content !p-6">
            <ModalHeader disabled={loading} title="Emergency Loan - Print All" sub="Manage emergency loan records." dismiss={dismiss} />
...
export default PrintAllEmergencyLoan;
