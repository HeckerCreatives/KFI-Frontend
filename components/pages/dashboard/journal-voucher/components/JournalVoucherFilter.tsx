...
type JournalVoucherFilterProps = {
  getJournalVouchers: (page: number, keyword?: string, sort?: string, from?: string, to?: string) => void;
};

const JournalVoucherFilter = ({ getJournalVouchers }: JournalVoucherFilterProps) => {
        ...
        useEffect(() => {
         ...
         const [data, setData] = useState<TData>({
            ...
            useEffect(() => {
              ...
              try {
                const result = await kfiAxios.get('/journal-voucher', { params: { limit: 5, search: code, page: 1 } });
          ...
          <div className=' flex flex-col gap-1'>
            <label htmlFor="search" className=' text-xs'>Search</label>
              <SearchInput
                  ...
                  suggestions={data.journalVouchers.map((item) => item.code || '')}
                />
          </div>
...
export default JournalVoucherFilter;
