...
import { DamayanFund, DamayanFundEntry, EmergencyLoan, EmergencyLoanEntry, TTableFilter } from '../../../../../types/types';
...
type UpdateELEntriesProps = {
  ...
  setPrevEntries: React.Dispatch<React.SetStateAction<DamayanFundEntry[]>>;
};
...
const UpdateDFEntries = ({ isOpen, damayanFund, setEntries, entries, deletedIds, setDeletedIds, setPrevEntries }: UpdateELEntriesProps) => {
  ...
  return (
    <div className="pb-2 h-full flex flex-col">
      <div>
        <AddEntry damayanFundId={damayanFund._id} getEntries={getEntries} entries={entries} setEntries={setEntries} setData={setData} transaction={damayanFund} />
      </div>
              ...
              currentPageItems.map((entry: DamayanFundEntry, index: number) => (
                <TableRow key={entry._id} className="border-b-0 [&>td]:border-4 [&>td]:!py-1 [&>td]:!px-2 [&>td]:!text-[.8rem]">
                  ...
                  <TableCell className="text-center space-x-1">
                    <UpdateEntry entry={entry} setData={setData} entries={entries} setEntries={setEntries} transaction={damayanFund} />
...
export default UpdateDFEntries;
