...
const ViewDFEntries = ({ isOpen, damayanFund }: ViewEntriesProps) => {
  const [present] = useIonToast();
  const [currentPage, setCurrentPage] = useState<number>(1);

  const entries = damayanFund.entries;
    const totalPages = Math.ceil(damayanFund.entries.length / TABLE_LIMIT);
      ...
      <div className="relative overflow-auto flex-1">
        <Table>
          ...
          <TableBody>
            ...
            {!data.loading &&
              paginatedEntries.map((entry: DamayanFundEntry, index: number) => (
      ...
      <div className="px-3">
       <div className="grid grid-cols-3">
         ...
         <div className="flex items-center justify-start gap-2 text-sm border-4 px-2 py-1 [&>div]:!font-semibold col-span-2">
           <div>Total: </div>
           <div>{`${damayanFund.amount.toLocaleString()}`}</div>
         </div>
       </div>
     </div>
    </div>
