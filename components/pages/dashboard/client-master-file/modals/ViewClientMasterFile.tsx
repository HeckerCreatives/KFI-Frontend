          ...
          <div className="overflow-y-auto pr-2 flex-1 space-y-6 mt-4">
            ...
            <div>
              <h3 className="text-sm !font-semibold text-slate-700 mb-3 flex items-center gap-1">
                ...
                <div className=' w-fit aspect-square overflow-hidden mb-6'>
                  <img width={100} height={100} src={`${process.env.NEXT_PUBLIC_IMG_URL}/${member.image?.path}`} alt="" className=''/>
                </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <ViewClientCard label="Children" containerClassName=' flex! flex-col!' value={member?.children.map((c, i) => `${i + 1}.${c.name}`).join('\n') || ''} />
                <ViewClientCard label="Beneficiary" value={member?.beneficiaries?.map((b, i) => `${i + 1}.${b.name}`).join('\n') || ''} />
              </div>
            </div>
