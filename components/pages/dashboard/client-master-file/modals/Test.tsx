...
const ExportAllClient = ({sort, search}:Props) => {
  ...
  return (
    <>
      <IonButton fill="clear" id="export_all_client_test" className="h-10 bg-orange-50 text-orange-500 border border-orange-200 capitalize font-semibold rounded-xl" strong>
        <FileExportIcon stroke='.8' size={15} className=' mr-2'/>
        Export
      </IonButton>
      <IonModal
        ref={modal}
        trigger={`export_all_client_test`}
