...
export type TEmergencyLoans = {
  emergencyLoans: { _id: string; code: string }[];
  totalPages: number;
  nextPage: boolean;
  prevPage: boolean;
  loading: boolean;
...
type EmergencyLoanSelectionProps<T extends FieldValues> = {
  setValue: UseFormSetValue<T>;
  clearErrors: UseFormClearErrors<T>;
  emergencyLoanLabel: Path<T>;
  emergencyLoanValue: Path<T>;
  className?: string;
};

const EmergencyLoanSelection = <T extends FieldValues>({ emergencyLoanLabel, emergencyLoanValue, setValue, clearErrors, className = '' }: EmergencyLoanSelectionProps<T>) => {
  ...
  const [data, setData] = useState<TEmergencyLoans>({
    emergencyLoans: [],
    loading: false,
    totalPages: 0,
    nextPage: false,
  ...
  const handleSelectExpenseVoucher = (journalVoucher: Option) => {
    ...
    setValue(emergencyLoanLabel as Path<T>, jvCode as any);
    setValue(emergencyLoanValue as Path<T>, jvId as any);
    clearErrors(emergencyLoanLabel);
    clearErrors(emergencyLoanValue);
    setData({
      emergencyLoans: [],
      loading: false,
      totalPages: 0,
...
export default EmergencyLoanSelection;
