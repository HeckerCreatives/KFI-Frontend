...
export type TDamayanFunds = {
  damayanFunds: { _id: string; code: string }[];
  totalPages: number;
  nextPage: boolean;
  prevPage: boolean;
  loading: boolean;
...
type DamayanFundSelectionProps<T extends FieldValues> = {
  setValue: UseFormSetValue<T>;
  clearErrors: UseFormClearErrors<T>;
  damayanFundLabel: Path<T>;
  damayanFundValue: Path<T>;
  className?: string;
};

const DamayanFundSelection = <T extends FieldValues>({ damayanFundLabel, damayanFundValue, setValue, clearErrors, className = '' }: DamayanFundSelectionProps<T>) => {
  ...
  const [data, setData] = useState<TDamayanFunds>({
    damayanFunds: [],
    loading: false,
    totalPages: 0,
    nextPage: false,
  ...
  const handleSelectExpenseVoucher = (journalVoucher: Option) => {
    ...
    setValue(damayanFundLabel as Path<T>, jvCode as any);
    setValue(damayanFundValue as Path<T>, jvId as any);
    clearErrors(damayanFundLabel);
    clearErrors(damayanFundValue);
    setData({
      damayanFunds: [],
      loading: false,
      totalPages: 0,
...
export default DamayanFundSelection;
