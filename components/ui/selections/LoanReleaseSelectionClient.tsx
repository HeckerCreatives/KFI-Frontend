...
const LoanReleaseSelectionClient = <T extends FieldValues>({ loanReleaseLabel, loanReleaseValue, setValue, clearErrors, className = '', id }: LoanReleaseSelectionProps<T>) => {
  ...
  const handleSearch = async (page: number) => {
    ...
    try {
      ...
      const result = await kfiAxios.get(`/transaction/selections/client/${id}`, { params: filter });
  ...
  return (
    <>
      ...
      <IonModal
        ...
        className=" [--border-radius:0.7rem] auto-height [--max-width:32rem] [--width:95%]"
      >
        {/* <IonHeader>
          ...
          <div className="relative overflow-auto">
            <Table>
              ...
              <TableBody>
                ...
                {!loading && data.loanReleases.length < 1 && <TableNoRows colspan={1} label="No loan release found" />}
                {!loading &&
...
export default LoanReleaseSelectionClient;
